...
Pendulum::Pendulum(float displayx, float displayy) {
  ...
  gravity = -9.8;                     
  srand( time(NULL) );
  theta1_knot = rand()%30+15;        // 15-45 degrees
  theta2_knot = rand()%30+15;        // 15-45 degrees
...
void Pendulum::updatePendulum(double time, double stepsize, double m1, double m2) {
  ...
  double u3soln = u4_nth+(delta/6.0)*(t0 + 2*t1 + 2*t2 + t3);
  ...
  double u4soln = u3_nth + (delta/6.0)*(w0 + 2*w1 + 2*w2 + w3);
